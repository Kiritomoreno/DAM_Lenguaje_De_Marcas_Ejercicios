<?xml-stylesheet type="text/xsl" href="eurovision.xslt"?>
<?xml version="1.0" encoding="UTF-8"?>
<eurovision>
   <!-- Insertamos el contenido del XML -->

	<candidatura>
    	<pais>Alemania</pais>
    	<titulo_original>Rockstars</titulo_original>
    	<titulo_castellano>Estrellas del rock</titulo_castellano>
    	<artista>Malik Harris</artista>
    	<idioma>Ingles</idioma>
   		<video>D3FN2umFaM8</video>
  	</candidatura>

  	<candidatura>
    	<pais>España</pais>
    	<titulo_original>SloMo</titulo_original>
    	<titulo_castellano>Cámara lenta</titulo_castellano>
    	<artista>Chanel</artista>
    	<idioma>Español<br>Ingles</idioma>
    	<video>TO2yGXVQCfo</video>
  	</candidatura>

  	<candidatura>
    	<pais>Finlandia</pais>
    	<titulo_original>Jezebel</titulo_original>
    	<titulo_castellano>Jezebel</titulo_castellano>
    	<artista>The Rasmus</artista>
    	<idioma>Ingles</idioma>
    	<video>XtMfC3vFbc</video>
  	</candidatura>

	<candidatura>
    	<pais>Italia</pais>
    	<titulo_original>Brividi</titulo_original>
    	<titulo_castellano>Escalofríos</titulo_castellano>
    	<artista>Mahmood & Blanco</artista>
    	<idioma>Italiano</idioma>
    	<video>4MgMhzfUmiA</video>
  	</candidatura>

	<candidatura>
    	<pais>Ucrania</pais>
    	<titulo_original>Stefania</titulo_original>
    	<titulo_castellano>Estefanía</titulo_castellano>
    	<artista>Kalush Orchstra</artista>
    	<idioma></idioma>
    	<video>UiEGVYOruLk</video>
  	</candidatura>
	

</eurovision>
